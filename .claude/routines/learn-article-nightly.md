...
5. For a cluster row: replace the `pillar_mention` sentence in the pillar's file of that locale with a sentence linking the new article, and re-check the pillar. If the row has no `pillar_mention` (the Italian and English rows do not have one yet), find the sentence that announces this cluster's topic in the pillar and replace that, or add a sentence with the link where it belongs. A pillar that does not link every published cluster of its locale fails the publish check. For a pillar row: write one announcement sentence per planned cluster of that locale and copy each sentence verbatim into that cluster's queue row as `pillar_mention`.
