...
8. Put the Resend API key into the Supabase Edge Function secrets (Dashboard → Edge Functions → Secrets, `RESEND_API_KEY`) so `delete-account` can send a confirmation email; and note the Supabase Auth email templates (Dashboard → Auth → Email Templates, not in the repo) look wrong in dark mode (Ricardo, 2026-09-21).
...
11. Deletion confirmation email from `delete-account` (sent before the account goes, en/nl/it, via Resend from info@projectfood.dev) once item 8 is done; in the same pass restyle the Supabase Auth email templates so they read in dark mode (light background forced, dark text, no transparent logo on assumed white). Not blocked otherwise; the in-app receipt screen shipped 2026-09-21.
12. Play internal testing → closed testing (12 testers × 14 days); iOS build + TestFlight once Apple is enrolled.
