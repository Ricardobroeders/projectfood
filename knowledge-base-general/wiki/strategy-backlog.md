...
10. Create the nightly article routine (2026-09-25, Ricardo: "every day an article"). Everything it needs is in the repo; only the routine itself is left, and Claude cannot create it (the API call is blocked by the auto-mode classifier). Settings and the prompt to paste: `.claude/routines/learn-article-nightly.md`. Create it at https://claude.ai/code/routines or with `/schedule` in an interactive session: daily 03:00, repo `Ricardobroeders/projectfood`, model Fable 5.1, Default environment, **Supabase as the only connector** (a routine may call any tool of an included connector without asking), push notifications on. Then **Run now** once and read the session before letting the schedule take over. The queue is `projectfood-app/content/learn/queue.json`, 17 rows: seven NL clusters, then the IT pillar and its three clusters, then the EN pillar and its five.
