...
8. Two secrets, one session: (a) put the Resend API key into the Supabase Edge Function secrets (Dashboard → Edge Functions → Secrets, `RESEND_API_KEY`) so `delete-account` can send a confirmation email; (b) create a personal access token (Dashboard → Account → Access Tokens) and run `SUPABASE_ACCESS_TOKEN=sbp_... node supabase/templates/push.mjs` to put the dark-mode-safe sign-in code emails live (templates in `supabase/templates/`, built 2026-09-21; or paste them by hand under Authentication → Email Templates). Then send yourself a code from the sign-in screen with the phone in dark mode.
