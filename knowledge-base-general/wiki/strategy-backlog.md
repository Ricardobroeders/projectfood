...
- **Pack opening, rare cards and rarity by desert (rows 4, 5, 9).** Ricardo's loyalty question,
  2026-09-26: why would anyone keep logging? His reference is MTG / Pokemon, where opening packs
  until a rare turns up is the satisfying part; applied here as Golden Sprouts earned by eating,
  spent on plant packs, opened for cards and rare facts or rare versions of a card. His own worry:
  digital taps are trivial to fake, an agent could tap all day. Brainstormed with Claude the same
  day, and the shape it landed in:
  - **What packs supply that the app lacks: anticipation.** Cards, albums and the ladder are all
    deterministic by design (rule 2, the next goal is always visible), so nothing in the product
    surprises anyone. That is the real hole the idea points at.
  - **A pack may never grant a plant card.** A card means "this kid tasted this plant", which is
    what makes the parent's recap true and what the paid Sunday advice runs on. Cards handed out by
    packs stop being evidence of eating, and the subscription loses the thing it sells. Randomness
    sits on top of a taste that really happened, never instead of one.
  - **Cheating has no payoff as long as the reward has no outside value** (Ricardo's own resolution,
    2026-09-26: "it's value for yourself"). Scalping exists in Pokemon because a pack is worth money
    outside the game. So the two things that would manufacture the problem are trading between
    households and Sprouts bought for money, both of which are on this list and both of which drop.
  - **The case that survives is a kid tapping for the shiny, not fraud.** Pace pulls per day with a
    dinner logged, never per plant: fifty taps earn what four earn. Same principle as the existing
    two-day rule for discovery rungs 2-4. Issue pulls server-side, seeded per (member, day), so a
    reinstall or a clock change cannot reroll. The PWA's bulk loggers (Rob 63 plants in a day, Imke
    51) both left, so bulk logging is a churn signal to read, not an exploit to police.
  - **Sprouts bought for money are out.** Packs plus purchasable currency plus a child-visible
    reward is a loot box in a family app: Play's Families policy requires odds disclosure, EU
    consumer regulators are tightening on virtual currency and minors, and "parent pays, child opens
    a random pack" is the pattern that draws press. It also breaks "accessories are earned, not
    bought" and pillar 4's "never paywall a child's reward". Monetise the advice that finishes the
    album instead.
  - **Sprouts as a currency at all are doubtful.** A currency needs a sink, the only sink named is
    the streak buy-back, and the free D7 freeze already absorbs a missed day; the XP lean in
    [[concept-achievement-system]] is already "drop for v1". A third currency for a solved problem.
  - **Rarity by desert, not by dice** (the strongest version, out of Ricardo's framing "you earned
    it because you ate something you don't like"). The rare card attaches to the hard bite: a plant
    marked refused and then tasted, a plant new to the whole household, one not seen in three
    months, one the kid chose at the shop. Unfakeable because it is measured against your own
    history, not a random number, and it steers behaviour, which was the brief. Keep the
    anticipation by splitting it: the **when** is earned, the **which** is a surprise.
  - **Content cost decides what "rare" can be.** A rare fact is new writing in five locales; a rare
    render is a new asset per plant through the n8n pipeline. The cheap rare is drawn in code, a
    foil treatment over the render that already exists: no assets, no words, every locale.
  - **Build order if it goes ahead:** (1) the Sunday pack, the week's already-earned cards revealed
  ...
  - **Card levels: decided on 2026-09-26, [[decision-2026-09-26-card-levels]].** Bronze 3, silver 8,
    gold 15 (a taste is one plant on one day), each placed so the first one lands before a churn
    moment: day 6 against the one-week exit that took 5 of 15 PWA users, day 24 against the four to
    five week exit, day 40 against the six to eight week exit. Every churned user would have left
    holding bronze cards. The card itself stays on the first taste. Platinum 25 and diamond 50 are
    agreed and held until there are cup renders. Ricardo's first proposal (10 / 25 / 50 with the card
    at bronze) was dropped because five of eight PWA members would have owned zero cards. Shipped the
    same day in `src/features/plants/cardLevel.ts`, with Regulars' first two rungs following the
    metals by name.
  - **Gold plant renders (Ricardo is drawing them).** The plant itself turns gold once a member's card
    reaches gold. Plumbing shipped 2026-09-26: drop PNGs in the bucket folder `food-images/gold/` at
    each plant's own file name, run `node scripts/build-assets.mjs`, and they appear on the per-member
    card tiles and on the plant page once anyone at the table is at gold; a plant without one keeps
    its normal render, so the set can grow one file at a time.
...
| 9 | Business model | drafted | Pick option A/B/C and confirm prices in [[concept-business-model]]; `households.plan` (free/family) exists since 2026-09-16, payments off in the store POC; input 2026-09-19: Golden Sprouts as the first store item, later purchasable; 2026-09-26: purchasable Sprouts dropped, packs plus bought currency plus a child-visible reward is a loot box in a family app (Play Families odds disclosure, EU rules on minors and virtual currency, pillar 4); monetise the advice that finishes the album; 2026-09-21: the app is published free to download, the paid plan is an in-app subscription added later (Play's "free can never become paid" concerns the download price only, no second app) |
