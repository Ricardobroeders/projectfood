...
## Next to pick up
_Maintained by Claude after every session; this is the answer to "what's next?". This is the one to-do list: decisions for Ricardo, build steps for Claude, and brainstorms to hold. Raw ideas live in the parking lot below until a brainstorm turns them into a decision or a build step. Updated 2026-09-21._

**Ricardo's decisions and accounts**
1. Store POC prerequisites (see [[decision-2026-09-16-store-poc-scope]]): custom SMTP (Resend) in Supabase Auth for the email code; Apple Developer enrolment (Individual) for iOS builds, Apple sign-in and TestFlight. Google sign-in is built in the app (native id-token flow, button on the sign-in screen since 2026-09-16) but dead until these accounts exist, in this order (2026-09-21): (a) a Firebase project "Project Food" with the Android app `dev.projectfood.app` → download `google-services.json` into `projectfood-mobile/` (FCM; gitignored); (b) in the Google Cloud project Firebase created: OAuth consent screen (external, support email, privacy and terms URLs), then a **Web** OAuth client with redirect URI `https://lkmfmdehysmbstnfdbyg.supabase.co/auth/v1/callback` → client id + secret into Supabase Auth → Providers → Google (also list the id under authorised client ids), and the client id into `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID` in `eas.json` (base env, public) and `.env.local`; (c) **Android** OAuth clients for `dev.projectfood.app` with two SHA-1s: the EAS keystore (`eas credentials -p android`, for preview/dev builds) and Play App Signing (Play Console → Setup → App signing, visible after the first AAB upload in item 12); these are matched server-side, no rebuild; (d) an iOS client once Apple is enrolled (`EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID` + `GOOGLE_IOS_URL_SCHEME`). The web client id is inlined at build time, so do (a)–(b) before Claude's first production AAB or accept one rebuild. Google sign-in is not a Play review requirement (the email code plus a password review account cover that), but it must work before the closed test starts. Store listing assets: the five Play screenshots and the feature graphic in Figma, from the brief in [[seo-app-store-aso]] (written 2026-09-20; make them after the accent decision in item 2, the listing must be complete before closed testing).
...
12. Play internal testing → closed testing (12 testers × 14 days); iOS build + TestFlight once Apple is enrolled. Before the listing goes up: reviewer access for the Play and Apple "App access" declaration, a password sign-in for one review account, because reviewers cannot receive the email code (2026-09-21). Sequence agreed 2026-09-21: Ricardo creates the Play app (done 2026-09-21 in another session: Project Food, `dev.projectfood.app`); Claude builds the first production AAB (`eas build -p android --profile production`; next build step, ideally after item 1 (a)–(b) so the Google button works in it, and with the Google button hidden while the web client id is empty so testers never see a dead button); Ricardo uploads it by hand to internal testing (Play requires the first bundle through the console, the `eas submit` service account comes later); that upload surfaces the Play app-signing SHA-1 for the Google OAuth client (item 1), and internal testers can install the store-signed build instead of the sideloaded preview APK. The 12-tester closed test only starts after the "Set up your app" tasks, so those get filled in alongside.
