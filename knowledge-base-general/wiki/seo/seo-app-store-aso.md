...
2. Read as a **family** app on the parent's phone, never a kids' app (no Kids Category, rated
   4+; file §3.3). Since 2026-09-20 this is also a Play policy matter: the app is declared for
   **ages 18 and over** (the parent holds the phone and the account) and stays out of Designed
   for Families. Play's target-audience policy then rejects a listing that carries "marketing
   elements that suggest otherwise (such as youthful animation or young characters in the
   graphic assets)"; the only ways out are to strip those elements or to declare an under-13
   audience and meet the whole Families policy. Google also warns that "imagery and terminology
   in your app that could be considered targeting children" feeds the same assessment. Rule:
   the listing is addressed to the adult. No "kids" or "children" in the app name, short
   description, screenshots or feature graphic; the words are the family and the table. No
   character avatars or child illustrations in the frames, initials and clay renders only. The
...
class-milestone and Sunday-advice frames wait for their features. Ricardo builds the frames in
...
get over months (4), what do we say at the table (5). Every headline is in the parent's voice,
second person, sentence case, no exclamation marks, no health claims, no "picky eater", and
the app is described as a thing the family does on the parent's phone, never as something for
children.
...
Five frames are the set; the sixth only if the carousel feels thin. Beyond five almost nobody
swipes, and frames 1 and 2 do most of the work.

**Copy per frame** (headline in bold, max two lines; sub-line one line; no "kids", "children",
"bimbi", "kinderen" anywhere on a frame):
...
| 6 | **One question, after dinner** / Half an hour after your dinner time. That is it. | **Eén vraag, na het eten** / Een half uur na jullie etenstijd. Meer niet. | **Una domanda, dopo cena** / Mezz'ora dopo la vostra cena. Tutto qui. |

**Template (one Figma component, five instances).** Frame 1080 × 1920. Background: one of the
icon's four pastels per frame, alternating so neighbouring frames differ. Headline Plus Jakarta
Sans ExtraBold 68 px, ink `#1F1B16`, centred, 120 px from the top, max two lines (the carousel
thumbnail is about half a phone wide, so anything under 60 px stops being readable). Sub-line
Medium 34 px, ink2 `#6B645C`. Phone from y ≈ 420, 820 px wide (screen 780 = 390 pt at 2×),
bleeding off the bottom edge, 20 px ink bezel, no shadow (design taste). Android status bar at
18:32. The UI inside is in the listing's language with the mock family below. The accent must be
the shipped one and row 2 (blue vs teal) is still open, so bind the frames to the theme variable
and re-export after that decision.

**Mock family and numbers** (the same across all frames of a set): parent + two members, shown
as initial discs on the member colours, no character avatars (the Bunny, Robot and Crocodile
heads are the "young characters" the Play reviewer looks for). EN Anna / Mia / Leo; NL Sanne /
Noor / Finn; IT Giulia / Sofia / Luca. Week 17 of 30, per member 14 / 11 / 9, streak 6 dinners.
Member 2: 23 foods tried, 6 silver, 2 gold, 11 of 72 levels. Strawberry tastes: member 2 six
...
**Play Console specs (checked 2026-09-20).** Phone screenshots: 2 to 8, JPEG or 24-bit PNG
without alpha, 1080 × 1920 (9:16). The long side may not exceed twice the short side, so
1080 × 2400 is rejected. At least four at 1080 px or more to be eligible for featuring. Feature
graphic 1024 × 500 is required: wordmark plus the tagline ("The family tastes, you tap" /
"Het gezin proeft, jij tikt" / "La famiglia assaggia insieme") on a pastel with three or four
clay renders, not the icon again. Icon 512 × 512 32-bit PNG (`assets/brand/store/play-icon-512.png`
