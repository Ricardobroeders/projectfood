...
export function Avatar({ username, size = 'md', border = 'default', className = '' }: Props) {
  const { outer, text } = SIZES[size]
  const ringColor = border !== 'default' ? BORDER_COLORS[border] : undefined

  return (
    <div
      className={`${outer} rounded-full bg-[#F5C518] flex items-center justify-center shrink-0 ${className}`}
