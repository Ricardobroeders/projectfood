-- Add custom_avatar_bg to all user-returning RPCs so avatar background colour shows everywhere

-- leaderboard (global plants)
DROP FUNCTION IF EXISTS leaderboard(int);
CREATE FUNCTION leaderboard(p_limit int DEFAULT 10)
RETURNS TABLE (rank int, username text, unique_plants int, is_me boolean, avatar_url text, active_border text, avatar_bg text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH ranked AS (
    SELECT
      ...
      us.avatar_url,
      us.active_border,
      us.custom_avatar_bg AS avatar_bg,
    ...
    GROUP BY us.user_id, us.username, us.avatar_url, us.active_border, us.custom_avatar_bg
  )
  SELECT rank, username, unique_plants, is_me, avatar_url, active_border, avatar_bg
...
RETURNS TABLE (rank int, username text, streak int, is_me boolean, avatar_url text, active_border text, avatar_bg text)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH all_days AS (
  ...
  SELECT rank, username, streak, is_me, avatar_url, active_border, avatar_bg
  FROM ranked ORDER BY rank LIMIT p_limit;
$$;
...
RETURNS TABLE (rank BIGINT, user_id UUID, username TEXT, unique_plants BIGINT, is_me BOOLEAN, avatar_url TEXT, active_border TEXT, avatar_bg TEXT)
...
RETURNS TABLE (rank BIGINT, user_id UUID, username TEXT, streak BIGINT, is_me BOOLEAN, avatar_url TEXT, active_border TEXT, avatar_bg TEXT)
LANGUAGE sql SECURITY DEFINER AS $$
  WITH friends AS (
    SELECT addressee AS fid FROM friendships WHERE requester = auth.uid() AND status = 'accepted'
    UNION SELECT requester FROM friendships WHERE addressee = auth.uid() AND status = 'accepted'
...
RETURNS TABLE (user_id UUID, username TEXT, week_count BIGINT, day_streak BIGINT, avatar_url TEXT, active_border TEXT, avatar_bg TEXT)
LANGUAGE sql SECURITY DEFINER AS $$
  WITH friend_ids AS (
    SELECT addressee AS fid FROM friendships WHERE requester = auth.uid() AND status = 'accepted'
  ...
  WHERE us.username IS NOT NULL
  GROUP BY us.user_id, us.username, us.avatar_url, us.active_border, us.custom_avatar_bg, ds.day_streak
...
RETURNS TABLE (id UUID, type TEXT, other_user_id UUID, username TEXT, avatar_url TEXT, active_border TEXT, avatar_bg TEXT)
LANGUAGE sql SECURITY DEFINER AS $$
  SELECT f.id, 'incoming'::TEXT, f.requester, us.username, us.avatar_url, us.active_border, us.custom_avatar_bg AS avatar_bg
  FROM friendships f JOIN user_settings us ON us.user_id = f.requester
  WHERE f.addressee = auth.uid() AND f.status = 'pending'
  UNION ALL
  SELECT f.id, 'outgoing'::TEXT, f.addressee, us.username, us.avatar_url, us.active_border, us.custom_avatar_bg AS avatar_bg
...
RETURNS TABLE (user_id UUID, username TEXT, total_plants BIGINT, week_count BIGINT, streak BIGINT, longest_streak_days INT, active_border TEXT, avatar_url TEXT, avatar_bg TEXT)
LANGUAGE sql SECURITY DEFINER AS $$
  WITH target AS (
    SELECT us.user_id, us.username, us.active_border, us.avatar_url, us.custom_avatar_bg AS avatar_bg
  ...
  GROUP BY t.user_id, t.username, t.active_border, t.avatar_url, t.avatar_bg;
$$;
GRANT EXECUTE ON FUNCTION user_profile(TEXT) TO authenticated;

-- search_users
DROP FUNCTION IF EXISTS search_users(TEXT);
CREATE FUNCTION search_users(p_query TEXT)
RETURNS TABLE (user_id UUID, username TEXT, total_plants BIGINT, avatar_url TEXT, avatar_bg TEXT)
LANGUAGE sql SECURITY DEFINER AS $$
  SELECT us.user_id, us.username, COUNT(DISTINCT pl.plant_id) AS total_plants, us.avatar_url, us.custom_avatar_bg AS avatar_bg
  FROM user_settings us
  LEFT JOIN plant_logs pl ON pl.user_id = us.user_id
  ...
  GROUP BY us.user_id, us.username, us.avatar_url, us.custom_avatar_bg
