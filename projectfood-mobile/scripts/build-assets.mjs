...
//   node scripts/build-assets.mjs            plants + avatars + stamps + cups + categories + notification icon
...
// Gold renders of the same plants, same file name under food-images/gold/ (Ricardo, 2026-09-26: the
// plant itself turns gold once a member's card reaches gold). A plant without one falls back to its
// normal render, so the set can grow one file at a time.
const PLANTS_GOLD_URL = `${'https://lkmfmdehysmbstnfdbyg.supabase.co/storage/v1/object/food-images'}/gold`;
...
// Card-level cups from images/app-ui-images (Ricardo, 2026-09-18); thresholds in src/features/plants/cardLevel.ts.
...
async function run() {
  ...
  mkdirSync(join(ROOT, 'assets/plants-gold'), { recursive: true });
  ...
  if (!skipDownload) {
    ...
    for (const [cat, file] of Object.entries(CATEGORIES)) {
      const dest = join(ROOT, `assets/categories/${cat}.webp`);
      if (existsSync(dest)) continue;
      try {
        const src = await fetchCached(`${CUPS_URL}/category-${file}.png`, `category-${cat}.png`);
        await toWebp(src, dest);
  ...
  const goldLines = catalog
    .filter((p) => existsSync(join(ROOT, `assets/plants-gold/${p.slug}.webp`)))
    .map((p) => `  '${p.slug}': require('@/assets/plants-gold/${p.slug}.webp'),`);
  writeFileSync(
    join(ROOT, 'src/data/plantGoldImages.generated.ts'),
  ...
  const categoryLines = Object.keys(CATEGORIES).filter((c) => existsSync(join(ROOT, `assets/categories/${c}.webp`))).map((c) => `  ${c}: require('@/assets/categories/${c}.webp'),`);
  writeFileSync(
    join(ROOT, 'src/data/categoryImages.generated.ts'),
  ...
  process.stdout.write(`done: ${plantLines.length} plants, ${goldLines.length} gold, ${avatarLines.length} avatars, ${stampLines.length} stamps, ${cupLines.length} cups, ${categoryLines.length} categories, ${failed} failed\n`);
